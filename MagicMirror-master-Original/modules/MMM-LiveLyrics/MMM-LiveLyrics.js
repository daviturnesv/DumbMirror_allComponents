...
Module.register("MMM-LiveLyrics", {
	...
	start () {
		...
		if (this.config.hideStrategy) this.config.hideStrategy = this.config.hideStrategy.toLowerCase();
		...
		this.builder = new LyricsDomBuilder(this.file(""), this.config, (a, b) => this.translate(a, b));
	...
	getDom () {
		...
		if (this.onSpotifyVersionMismatch) return this.builder.warning(
		...
		if (this.helpOverlay) return this.builder.help(
			this.translate(this.server.title),
			this.server.url,
			this.server.url
		);
		const dom = this.builder.paint(this.current, this.lyrics);
		this.decorateLyricsDom(dom, this.lyrics);
		return dom;
	},
	...
	decorateLyricsDom (dom, lyrics) {
		if (!dom || !lyrics || !lyrics.lyrics) return;

		const container = dom.querySelector(`#${this.builder.LYRICSCONTAINER}`);
		const lyricBlock = container?.querySelector(
			`#${this.builder.LYRICSSCROLLABLE}`
		);
		if (!container || !lyricBlock || container.querySelector(".meta")) return;

		const raw = lyricBlock.textContent;
		const { meta, body } = this.extractMetadataBlock(raw);

		container.classList.remove("meta-present", "meta-empty");

		if (meta) {
			const metadata = document.createElement("div");
			metadata.classList.add("meta");
			metadata.textContent = meta;
			lyricBlock.textContent = body;
			container.classList.add("meta-present");
			container.insertBefore(metadata, lyricBlock);
		} else {
			container.classList.add("meta-empty");
		}
	},

	extractMetadataBlock (rawLyrics) {
		if (typeof rawLyrics !== "string") return { meta: "", body: rawLyrics ?? "" };

		const normalized = rawLyrics.trim();
		if (!normalized) return { meta: "", body: "" };
		...
		let bracketMatch;
		while (!detectedSection && (bracketMatch = bracketRegex.exec(normalized)) !== null) {
		...
		if (detectedSection && detectedSection.index > 0) {
			const candidateMeta = normalized.slice(0, detectedSection.index).trim();
			const candidateBody = normalized.slice(detectedSection.index).trim();
		...
		if (!isMetadata) return { meta: "", body: normalized };
		...
		this.config.events[notification]?.split(" ").forEach((e) => {
			switch (e) {
				case "NOW_PLAYING":
					...
					if (this.config.updateTopModulesCalcOnData) this.builder.getTopModulesHeight();
					break;
				case "LYRICS_SHOW":
					...
					if (this.config.startHidden) {
						setTimeout(
							() => {
								...
								this.enable = true;
								if (!this.config.showConnectionQrOnLoad) this.builder.hideLoading();
							},
	...
	suspend () {
		...
		if (this.config.logSuspendResume && !this.firstSuspend) Log.info(
	...
	resume () {
		...
		if (this.config.logSuspendResume) Log.info(
