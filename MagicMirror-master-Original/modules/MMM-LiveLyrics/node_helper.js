...
const readDotEnv = () => {
  ...
  try {
    ...
    console.error("[MMM-LiveLyrics] Failed to read .env for Genius token", e);
    dotenvCache = {};
  }
  return dotenvCache;
};
  ...
  socketNotificationReceived: async function (notification, payload) {
    switch (notification) {
      case "SET_CREDENTIALS": {
        const tokenFromPayload = payload?.apiKey;
        const resolvedToken = tokenFromPayload || resolveEnv("LIVELYRICS_GENIUS_TOKEN");
        if (!resolvedToken) {
          console.warn(
            "[MMM-LiveLyrics] Genius token missing; set accessToken in config or LIVELYRICS_GENIUS_TOKEN in environment",
          );
        }
        ...
        console.log(
          "[MMM-LiveLyrics] Genius token source",
          tokenFromPayload ? "config" : "environment",
        );
        ...
        this.fetcher = new LyricsFetcher(mergedCredentials);
        this.currentStatus = {
          empty: true,
          dataType: "STATUS",
