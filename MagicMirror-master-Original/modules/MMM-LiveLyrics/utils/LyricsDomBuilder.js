...
class LyricsDomBuilder {
	constructor (pathPrefix, config, translator) {
		...
		if (!this.config.useAnimations) this.root.style.setProperty("--LILY-MAIN-TRANSITIONS", "none");
	...
	paint (current, lyrics) {
		const wrapper = this.globalWrapper();
		const gridOnBackdrop = this.usesBackdropGrid();
		const main = this.buildMainContainer(current, lyrics);
		const layout = this.buildBaseStructure();
		const media = this.buildMediaSection(current);
		const section = this.buildLyricsSection(current, lyrics);

		this.lyStatus = section.hasLyrics;

		if (section.banner) {
			layout.view.appendChild(section.banner);
			layout.view.appendChild(media);
		} else {
			this.appendLyricsContent(section);
			layout.view.appendChild(section.container);
			layout.view.appendChild(media);
		}

		layout.blobs.appendChild(layout.grid);
		layout.sub.appendChild(layout.blobs);
		layout.sub.appendChild(layout.view);
		main.appendChild(layout.sub);
		if (gridOnBackdrop) layout.backdrop.appendChild(layout.grid);
		main.appendChild(layout.backdrop);
	...
	usesBackdropGrid () {
		return (
			this.config.lyricsStyleTheme
			&& this.config.lyricsStyleTheme.toLocaleLowerCase() === "dynamicblobsfull"
		);
	}

	buildMainContainer (current, lyrics) {
		const main = document.createElement("div");
		this.getMainClasses(current, lyrics).forEach((cls) => main.classList.add(cls));
		return main;
	}

	getMainClasses (current, lyrics) {
		return [
			"main",
			this.types.style,
			this.types.scroll,
			...
			this.getSizingClass(),
			this.getBackdropClass(),
			this.getLayoutClass(),
			this.resolveLyricState(current, lyrics),
			this.resolvePlayerState(current)
		];
	}

	getVisibilityClass () {
		const strategy = this.config.hideStrategy;
		return strategy
			? `internal-visibility-${strategy.toLowerCase()}`
			: "internal-visibility-unknown";
	}

	getSizingClass () {
		return this.config.lyricsCustomFixedDimentions
			? "internal-custom-reset"
			: "internal-default-reset";
	}

	getBackdropClass () {
		return this.config.blurToBlackOnFull
			? "internal-use-blackout"
			: "internal-no-blackout";
	}

	getLayoutClass () {
		return this.config.sideBySideOnLandscape
			? "internal-sidebyside-normal"
			: "internal-sidebyside-disabled";
	}

	resolveLyricState (current, lyrics) {
		if (!current) return "unknown";
		if (!lyrics) return "searching";
		if (!lyrics.lyrics) return "missing";
		return "found";
	}

	resolvePlayerState (current) {
		if (!current) return "upstream-unknown";
		return current.playerIsEmpty ? "upstream-empty" : "upstream-playing";
	}
	...
	buildBaseStructure () {
		...
		return { backdrop, sub, view, blobs, grid };
	}

	buildMediaSection (current) {
		const media = document.createElement("div");
		media.classList.add("media");

		const cover = document.createElement("div");
		cover.classList.add("cover");
		cover.style.backgroundImage = `url(${this.getCoverImage(current)})`;
		...
		title.innerText = this.getTitleText(current);

		const subtitle = document.createElement("div");
		subtitle.classList.add("subtitle");
		subtitle.innerText = this.getSubtitleText(current);
	...
	getCoverImage (current) {
		if (!current || current.playerIsEmpty) return `/${this.pathPrefix}${this.assets.unknown}`;
		return current.image || `/${this.pathPrefix}${this.assets.unknown}`;
	}

	getTitleText (current) {
		if (!current || current.playerIsEmpty) return this.translate("UNKNOWN_TOP");
		const title = typeof current.name === "string" ? current.name.trim() : "";
		return title || this.translate("UNKNOWN_TOP");
	}

	getSubtitleText (current) {
		if (!current || current.playerIsEmpty) return this.translate("UNKNOWN_SUB");
		const artists = typeof current.artists === "string" ? current.artists.trim() : "";
		return artists || this.translate("UNKNOWN_SUB");
	}

	buildLyricsSection (current, lyrics) {
		const container = document.createElement("div");
		container.classList.add("container");
		container.id = this.LYRICSCONTAINER;

		const lyricsContent = document.createElement("div");
		lyricsContent.classList.add("ly");
		lyricsContent.id = this.LYRICSSCROLLABLE;

		if (!lyrics) {
			const searchContainer = this.createSearchContainer(current);
			return {
		...
		if (!lyrics.lyrics) {
			return {
				...
				lyrics: lyricsContent,
				metadata: null,
				banner: this.createNoLyricsBanner(),
				hasLyrics: false
			};
		}

		const { meta, body } = this.extractLyricsMetadata(lyrics.lyrics);
		lyricsContent.innerText = body;

		let metadata = null;
		if (meta) {
			metadata = document.createElement("div");
			metadata.classList.add("meta");
			metadata.innerText = meta;
			container.classList.add("meta-present");
		} else {
			container.classList.add("meta-empty");
		}
	...
	appendLyricsContent (section) {
		const target = section.container;
		if (!target) return;
		if (section.metadata) target.appendChild(section.metadata);
		target.appendChild(section.lyrics);
	}

	extractLyricsMetadata (rawLyrics) {
		if (typeof rawLyrics !== "string") return { meta: "", body: rawLyrics ?? "" };

		const sanitized = rawLyrics.trim();
		if (!sanitized) return { meta: "", body: "" };
		...
		const cleanMeta = (text) => {
			...
			while (sanitizedLines.length > 0) {
				...
				if ((/^\d[\d,.]*\s*contributors?/i).test(first)) {
					sanitizedLines.shift();
					continue;
				}
				if ((/lyrics["”]?$/i).test(first)) {
					sanitizedLines.shift();
					continue;
				}
				break;
			...
			return joined || text.replace(/\bRead\s+More\b:?/gi, "").trim();
		};

		let detectedSection = null;
		const bracketRegex = /\[[^\]]+\]/g;
		let bracketMatch;
		while (!detectedSection && (bracketMatch = bracketRegex.exec(sanitized)) !== null) {
			const tag = bracketMatch[0].toLowerCase();
			if (sectionKeywords.some((keyword) => tag.includes(keyword))) {
				detectedSection = bracketMatch;
			}
		}

		if (detectedSection && detectedSection.index > 0) {
			const candidateMeta = sanitized.slice(0, detectedSection.index).trim();
			const candidateBody = sanitized.slice(detectedSection.index).trim();
			const metaLooksValid
				= candidateMeta
				  && !candidateMeta.startsWith("[")
				  && (metadataHints.some((hint) => hint.test(candidateMeta))
				    || candidateMeta.split(/\s+/).length >= 6);

			if (metaLooksValid && candidateBody) {
				return { meta: cleanMeta(candidateMeta), body: candidateBody };
			}
		}
		...
		const blocks = sanitized
			...
			.map((part) => part.trim())
			.filter(Boolean);
		if (blocks.length === 0) return { meta: "", body: "" };

		const firstBlock = blocks[0];
		const looksLikeSection = (/^\[[^\]]+\]/).test(firstBlock);

		const isMetadata
			= !looksLikeSection
			  && blocks.length > 1
			  && firstBlock.length <= 400
			  && metadataHints.some((hint) => hint.test(firstBlock));

		if (!isMetadata) return { meta: "", body: sanitized };

		const body = blocks.slice(1).join("\n\n").trim();
		return { meta: cleanMeta(firstBlock), body };
	}

	createNoLyricsBanner () {
		const wrapper = document.createElement("div");
		...
		text.innerText = this.translate("LYRICS_EMPTY");

		nolyrics.appendChild(icon);
		nolyrics.appendChild(text);
		wrapper.appendChild(nolyrics);
		return wrapper;
	}

	createSearchContainer (current) {
		if (!current) return null;
		const search = document.createElement("div");
		search.classList.add("search");

		if (!current.playerIsEmpty) {
			...
			const text = document.createElement("div");
			text.classList.add("txt");
			text.innerText = this.translate("LYRICS_SEARCH");
