...
module.exports = class LyricsFetcher {
  ...
  async getLyrics(title, artists, artist) {
    try {
      ...
      if (!web || !web.response) {
        const status = web && web.meta ? web.meta.status : "unknown";
        const message = web && web.meta ? web.meta.message : "no meta";
        console.error(
          "[MMM-LiveLyrics] Genius search response missing payload",
          status,
          message,
        );
      }
  ...
  lyrics = lyrics.length > 1 ? lyrics.trim() : null;
