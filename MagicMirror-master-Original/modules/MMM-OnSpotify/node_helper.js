...
function readEnvFile() {
  ...
  try {
    ...
    console.warn("[MMM-OnSpotify] Unable to read .env for credential fallback", error);
  }
  return cachedEnv;
}
  ...
  socketNotificationReceived: function (notification, payload) {
    switch (notification) {
      case "SET_CREDENTIALS_REFRESH": {
        ...
        payload.credentials = {
          clientId: withFallback(payload?.credentials?.clientId, "ONSPOTIFY_CLIENT_ID"),
          clientSecret: withFallback(payload?.credentials?.clientSecret, "ONSPOTIFY_CLIENT_SECRET"),
          accessToken: withFallback(payload?.credentials?.accessToken, "ONSPOTIFY_ACCESS_TOKEN"),
          refreshToken: withFallback(payload?.credentials?.refreshToken, "ONSPOTIFY_REFRESH_TOKEN"),
          experimentalCanvasSPDCookie: payload?.credentials?.experimentalCanvasSPDCookie || "",
        };
        payload.secondaryCredentials = {
          clientId: withFallback(payload?.secondaryCredentials?.clientId, "ONSPOTIFY_SECONDARY_CLIENT_ID"),
          clientSecret: withFallback(payload?.secondaryCredentials?.clientSecret, "ONSPOTIFY_SECONDARY_CLIENT_SECRET"),
          accessToken: withFallback(payload?.secondaryCredentials?.accessToken, "ONSPOTIFY_SECONDARY_ACCESS_TOKEN"),
          refreshToken: withFallback(payload?.secondaryCredentials?.refreshToken, "ONSPOTIFY_SECONDARY_REFRESH_TOKEN"),
        };
