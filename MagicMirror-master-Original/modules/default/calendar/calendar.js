...
Module.register("calendar", {
	...
	start () {
		...
		this.config.calendars.forEach((calendar) => {
			...
			// Make sure the backend fetcher has a sufficiently high cap so we can expand via voice later.
			const minBackendEntries = Math.max(100, this.config.maximumEntries || 10);
			if (!calendarConfig.maximumEntries || calendarConfig.maximumEntries < minBackendEntries) {
				calendarConfig.maximumEntries = minBackendEntries;
			}
		...
		// keep base values for quick resets via voice
		this._baseView = {
			limitDays: this.config.limitDays,
	...
	notificationReceived (notification, payload, sender) {
		...
		// Custom: dynamically adjust how many days/entries are shown (voice control)
		else if (notification === "CALENDAR_SET_VIEW") {
			if (payload && typeof payload === "object") {
				if (typeof payload.limitDays === "number") this.config.limitDays = Math.max(0, payload.limitDays);
				// allow temporarily increasing the number of entries beyond the default 10 when requested via voice
				if (typeof payload.maximumEntries === "number") this.config.maximumEntries = Math.max(1, Math.min(200, payload.maximumEntries));
				this.updateDom(300);
			}
		}
		else if (notification === "CALENDAR_RESET_VIEW") {
			this.config.limitDays = this._baseView.limitDays;
			this.config.maximumEntries = this._baseView.maximumEntries;
			this.updateDom(300);
		}
	},
